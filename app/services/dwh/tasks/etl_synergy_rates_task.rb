class Dwh::Tasks::EtlSynergyRatesTask < Dwh::Tasks::BaseSynergyTask
  queue_as :default
          ...
          if employed == true
            ...
            unless start_date.blank? or start_date == "00000000"
              ...
              if valid_hr_data.blank? || start_date_as_date > Date.current
                # Send error log to notify of missing hr data
                Dwh::DgQualityLog.create!(quality_check_id: nil, result: "failed", error_message: "Geen HR data aanwezig voor: #{dim_user.full_name} (ID: #{dim_user.original_id})")
              else
                # Get company
