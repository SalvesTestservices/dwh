class Dwh::Tasks::EtlBbUsersTask < Dwh::Tasks::BaseTask
  queue_as :default
    ...
    begin
      ...
      ActsAsTenant.without_tenant do
        ...
        companies.where(id:13).each do |company|
          ...
          dim_company = Dwh::DimCompany.find_by(original_id: company.id)
          expected    = company.users.where.not(role: get_excluded_roles)          

          # Data quality check: row count
          actual = Dwh::DimUser.where(account_id: dim_account.id, company_id: dim_company.id).count
          ...
          #actual = Dwh::DimUser.where(account_id: dim_account.id, company_id: dim_company.id).where("role != 'Subco' AND contract_hours IS NOT NULL AND contract_hours > 0").count
