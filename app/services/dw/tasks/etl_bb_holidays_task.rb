class Dwh::Tasks::EtlBbHolidaysTask < Dwh::Tasks::BaseTask
  queue_as :default
        ...
        accounts.each do |account|
          ...
          companies.each do |company|
            ...
            holidays.each do |holiday|
              ...
              Dwh::EtlStorage.create(account_id: account.id, identifier: "holidays", etl: "transform", data: holidays_hash)
            end
          end
          ...
          holidays = Dwh::EtlStorage.where(account_id: account.id, identifier: "holidays", etl: "transform")
          unless holidays.blank?
            holidays.each do |holiday|
              dim_company = Dwh::DimCompany.find_by(account_id: dim_account.id, original_id: holiday.data['company_id'])
              unless dim_company.blank?
                uid = "#{dim_account.id}#{dim_company.id}#{holiday.data['holiday_date']}"
                Dwh::DimHoliday.upsert({ uid: uid, account_id: dim_account.id, company_id: dim_company.id, holiday_date: holiday.data['holiday_date'], name: holiday.data['name'], country: holiday.data['country'] }, unique_by: [:uid])
              end
              holiday.destroy
            end
          end
